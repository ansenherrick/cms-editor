...
the iPhone app's Publish action changes only the CMS item status. The post list
menu has a separate Deploy site action that calls Framer `publish()` and
`deploy()` so the configured custom domain receives the change. This deploys
every unpublished Framer project change, not only the current blog post.
